...
import GameOverCard from './Component/GameOverCard'
...
const initialState = {
  imageVisible: imagesList[0].imageUrl,
...
class App extends Component {
  ...
  onChangeImage = imgUrl => {
    ...
    const isMatched = imgUrl === imageVisible
    if (isMatched) {
      const len = imagesList.length
      const val = Math.ceil(Math.random() * (len - 1))
      const imageChange = imagesList[val].imageUrl
      this.setState(prevState => ({
        imageVisible: imageChange,
  ...
  render() {
    ...
    const altValue = isGameInProgress ? 'match' : ''
    console.log(isGameInProgress)
    const filteredImagesList = imagesList.filter(
      eachCategory => eachCategory.category === activeTabId,
    )
        ...
        <div className="main-container">
          <div className="container-1">
            ...
            {isGameInProgress === false && (
              <div>
                <GameOverCard score={score} onReset={this.onReset} />
              </div>
            )}
          </div>
        </div>
